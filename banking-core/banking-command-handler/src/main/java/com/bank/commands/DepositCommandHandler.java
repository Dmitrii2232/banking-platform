...
public class DepositCommandHandler {
    ...
    public List<Event> handle(DepositCashCommand cmd) {
        int retries = 0;
        while (retries < MAX_RETRIES) {
            try {
                ...
                retries++;
                log.warn("Конфликт версий при пополнении, попытка {}/{}: {}", retries, MAX_RETRIES, e.getMessage());
                if (retries >= MAX_RETRIES) {
                    throw new RuntimeException("Не удалось выполнить пополнение после " + MAX_RETRIES + " попыток", e);
                }
                try { Thread.sleep((long) Math.pow(2, retries) * 50); }
                catch (InterruptedException ie) { Thread.currentThread().interrupt(); break; }
            }
        }
        throw new RuntimeException("Не удалось выполнить пополнение");
    }

    @Transactional
    protected List<Event> doHandle(DepositCashCommand cmd) {
        log.info("Обработка пополнения: product={}", 
        ...
        BankProduct product = eventStore.loadProduct(cmd.getProductId())
            .orElseThrow(() -> new IllegalArgumentException("Продукт не найден: " + cmd.getProductId()));
        ...
        // ✅ Transactional Outbox: пишем в БД вместо прямой отправки в Kafka
